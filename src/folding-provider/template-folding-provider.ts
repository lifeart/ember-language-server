  ...
  handle(document: TextDocument): FoldingRange[] | null {
    ...
    traverse(ast, {
      BlockStatement(node: ASTv1.BlockStatement) {
        ...
        results.push(FoldingRange.create(loc.start.line, loc.end.line, loc.start.column, loc.end.column, FoldingRangeKind.Region));
      ...
      MustacheCommentStatement(node: ASTv1.MustacheCommentStatement) {
        ...
        results.push(FoldingRange.create(loc.start.line, loc.end.line, loc.start.column, loc.end.column, FoldingRangeKind.Comment));
      },
